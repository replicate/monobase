...
if [ -z "${COG_VERSION:-}" ]; then
    ...
    COG_PATH="$MONOBASE_PREFIX/cog/latest/cog$name-python$PYTHON_VERSION"
    if [ ! -d "$COG_PATH" ]; then
...
# Include both CuDNN 8 & 9
CUDNN_PATH="$MONOBASE_PATH/cudnn$CUDNN_VERSION-cuda${CUDA_MAJOR}"
CUDNN9_PATH="$MONOBASE_PATH/cudnn9-cuda${CUDA_MAJOR}"
CUDNN8_PATH="$MONOBASE_PATH/cudnn8-cuda${CUDA_MAJOR}"
...
export LD_LIBRARY_PATH="/usr/lib/x86_64-linux-gnu:$CUDA_PATH/lib64:$CUDNN9_PATH/lib:$CUDNN8_PATH/lib:${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
